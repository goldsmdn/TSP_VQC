...
def estimate_cost_fn_gradient(
    my_input: torch.Tensor, output: torch.Tensor, cost_fn: Callable[[list], int]
) -> torch.Tensor:
    ...
    def _init_weights(self, fc, first_layer: bool = False):
        ...
        with torch.no_grad():
            if not self.hot_start:
                ...
                fc.weight.add_(
                    torch.normal(mean=0.0, std=self.std_dev, size=fc.weight.shape)
                )
                fc.bias.copy_(
                    torch.normal(mean=0.0, std=self.std_dev, size=fc.bias.shape)
                )
