...
def find_maximum_qubits_needed(qubit_dict: dict) -> int:
    """find the maximum number of qubits needed for Qiskit circuit set up"""
    maximum_qubits_needed = max(qubit_dict.values()) + 1
    # lowest qubit is 0, highest is max(qubit_dict.values())
    # the lowest qubit is always 0
    return maximum_qubits_needed
...
def mode_7(context_dict: dict) -> Circuit:
    ...
    print(
        f'After circuit set up, the verbatim box receives the following circuit{inner.qubits}'
    )
...
def mode_13(context_dict: dict) -> Circuit:
    ...
    for layer in range(layers):
        ...
        for i in range(qubits_measured):
            inner.rz(
                qubit_dict[i],
                params[qubits_measured + i + offset],
            )
            # hadamdard
            inner.rz(qubit_dict[i], np.pi / 2)
            inner.rx(qubit_dict[i], np.pi / 2)
...
def mode_20(context_dict: dict) -> QuantumCircuit:
    """Creates a simple Braket circuit with based on Mode 2, which supports the hot start.
    ...
    assert len(qubit_dict) >= qubits_measured

    required_params = layers * qubits_measured * 2
    assert len(params) >= required_params

    qc = Circuit()
    for layer in range(layers):
        print(f'{qubit_dict=}, {qubits_measured=}')
        offset = layer * qubits_measured * 2
        for i in range(qubits_measured):
            theta1 = params[i + offset]
            qc.rx(
        ...
        for i in range(qubits_measured):
            theta2 = params[qubits_measured + i + offset]
            q1, q2 = (
                qubit_dict[i % qubits_measured],
                qubit_dict[(i + 1) % qubits_measured],
            )
            qc.xx(
                q1,
                q2,
    ...
    print(qc)
    print('Instruction count:', len(qc.instructions))
...
def mode_21(context_dict: dict) -> QuantumCircuit:
    """Creates a simple Braket circuit with based on Mode 2, which supports the hot start.
    This is based on circuit 20, but removing the non standard gates.
...
def mode_22(context_dict: dict) -> QuantumCircuit:
    """Creates a simple qiskit circuit based on the Cephus compatible version
    of Circuit 2, so that this can be simulated using the Qiskit MPS simulator.
    ...
    required_params = layers * qubits_measured * 2
    print(f'{len(params)=}, {required_params=}, {qubits_measured=}')
    assert len(params) >= required_params
    ...
    for layer in range(layers):
        print(f'{qubit_dict=}, {qubits_measured=}, {qubits_needed=}')
        offset = layer * qubits_measured * 2
        for i in range(qubits_measured):
            theta1 = params[i + offset]
            # print(f'{qubit_dict[i]=}')
            qc.rx(
                theta1,
                qubit_dict[i],
            )
